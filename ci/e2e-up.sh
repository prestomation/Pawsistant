#!/usr/bin/env bash
# One-shot local / session runner for the card e2e tests:
#   prepare env -> build card -> start HA -> wait -> run Playwright -> tear down.
#
...
echo "[e2e-up] building card JS..."
bash ci/build-card.sh
