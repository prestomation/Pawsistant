...
class PawsistantStore:
    ...
    # -----------------------------------------------------------------------
    # A care schedule links a (dog, event_type) pair to a recurring task in the
    # optional Home Keeper integration. The schedule dict holds the recurrence
    # config plus the Home Keeper ``task_id`` (or None if Home Keeper was absent
    # when it was created). Keyed by a schedule_id (uuid) generated by the caller so
    # it can be embedded in the Home Keeper task's ``source`` before the task exists.

    def get_care_schedules(self) -> dict[str, dict[str, Any]]:
        """Return all care schedules as {schedule_id: {...}}."""
        return dict(self._meta.get(CONF_CARE_SCHEDULES, {}))

    def get_care_schedule(self, schedule_id: str) -> dict[str, Any] | None:
        """Return a single care schedule, or None."""
        return self._meta.get(CONF_CARE_SCHEDULES, {}).get(schedule_id)

    def find_care_schedule(
        self, dog_id: str, event_type: str
    ) -> tuple[str, dict[str, Any]] | None:
        """Return the (schedule_id, schedule) for a dog+event_type pair, or None.

        Only one schedule per pair is allowed, so the bidirectional mirror is
        unambiguous.
        """
        for schedule_id, schedule in self._meta.get(CONF_CARE_SCHEDULES, {}).items():
            if schedule.get("dog_id") == dog_id and schedule.get("event_type") == event_type:
                return schedule_id, schedule
        return None

    async def add_care_schedule(
        self, schedule_id: str, schedule: dict[str, Any]
    ) -> dict[str, Any]:
        """Persist a new care schedule under *schedule_id* and return it."""
        self._meta.setdefault(CONF_CARE_SCHEDULES, {})[schedule_id] = schedule
        await self._save_meta()
        return schedule
    ...
    async def update_care_schedule(
        ...
        """Apply *changes* to a care schedule and persist; return it (or None)."""
        schedule = self._meta.get(CONF_CARE_SCHEDULES, {}).get(schedule_id)
        if schedule is None:
            return None
    ...
    async def remove_care_schedule(self, schedule_id: str) -> dict[str, Any] | None:
        """Remove a care schedule and return it (or None if it didn't exist)."""
        schedule = self._meta.get(CONF_CARE_SCHEDULES, {}).pop(schedule_id, None)
        if schedule is not None:
            await self._save_meta()
        return schedule

    async def remove_care_schedules_for_dog(
        self, dog_id: str
    ) -> list[dict[str, Any]]:
        """Remove every care schedule for *dog_id*; return the removed schedules."""
        return await self._remove_care_schedules(lambda s: s.get("dog_id") == dog_id)

    async def remove_care_schedules_for_event_type(
        self, event_type: str
    ) -> list[dict[str, Any]]:
        """Remove every care schedule for *event_type*; return the removed ones."""
        return await self._remove_care_schedules(
            lambda s: s.get("event_type") == event_type
        )

    async def _remove_care_schedules(self, predicate) -> list[dict[str, Any]]:
        schedules = self._meta.get(CONF_CARE_SCHEDULES, {})
        doomed = [sid for sid, s in schedules.items() if predicate(s)]
        removed = [schedules.pop(sid) for sid in doomed]
        if removed:
            await self._save_meta()
        return removed
